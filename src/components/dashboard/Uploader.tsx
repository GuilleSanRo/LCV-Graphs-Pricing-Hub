      ...
      {error && (
        <div className="mt-3 flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">
          ...
          <div>
            ...
            {missing && (
              <div className="mt-1 text-xs">Missing columns: <span className="font-mono">{missing.map(m => m === "Market" ? "Market or Channel" : m === "Make" ? "Make or Brand" : m === "Segment" ? "Segment or/and Label or Cluster" : m).join(", ")}</span></div>
            )}
          </div>
        </div>
      )}
      {!dataset && !error && (
        <details className="mt-3 text-xs text-muted-foreground">
          ...
          <div className="mt-2 grid grid-cols-2 gap-1 font-mono md:grid-cols-3">
            {REQUIRED_COLUMNS.map((c) => <span key={c}>· {c === "Market" ? "Market or Channel" : c === "Make" ? "Make or Brand" : c === "Segment" ? "Segment or/and Label or Cluster" : c}</span>)}
          </div>
        </details>
      )}
    </div>
